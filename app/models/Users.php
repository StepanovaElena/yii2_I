...
class Users extends UsersBase implements IdentityInterface
{
    ...
    public function scenarioSingin(): self
    {
    ...
    {
        return array_merge([
            self::SCENARIO_SIGNUP => ['email_fld', 'password'],
            self::SCENARIO_SIGNIN => ['email_fld', 'password']
        ], parent::scenarios());
    }
    ...
    public function rules()
    {
        return array_merge([
            ['password', 'required'],
            ['password', 'string', 'min' => 6], //можно добавить регулярное выражение
            [['email_fld'], 'unique', 'on' => self::SCENARIO_SIGNUP],
            [['email_fld'], 'exist','on'=>self::SCENARIO_SIGNIN]
        ], parent::rules());
    }
    ...
    {
        return Users::find()->cache(20)->andWhere(['id_fld' => $id])->one();
    }
